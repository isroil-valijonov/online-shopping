...
public class OtpService {
    ...
    private final String VERIFICATION_MASSAGE = "Your verification code is:";
    ...
    private CommonResponse sendSms(String phoneNumber, int code){
        System.out.printf((VERIFICATION_MASSAGE), code);
        return new CommonResponse("Sms was sent successful", LocalDateTime.now(), HttpStatus.OK.value());
    }
    ...
    private OTP sendSmsInternal(String phoneNumber) {
        ...
        smsNotificationService.sendNotification(phoneNumber, VERIFICATION_MASSAGE + code);
        return new OTP(phoneNumber, code,  LocalDateTime.now(), 1, LocalDateTime.now(), false);
    }

    private OTP sendSmsInternal(OTP otp) {
        int code = random.nextInt(100000, 999999);
        smsNotificationService.sendNotification(otp.getPhoneNumber(), VERIFICATION_MASSAGE + code);
        otp.setCode(code);
